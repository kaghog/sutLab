...
import seville.data.hts.entd.cleaned
...
The only change is the income class calculation method.
"""

INCOME_CLASS = {
    "entd": seville.data.hts.entd.cleaned.calculate_income_class,
}
...
def statistical_matching(progress, df_source, source_identifier, weight, df_target, target_identifier, columns, random_seed = 0, minimum_observations = 0):
    ...
    if cdf.size == 0:
        print("No umatched obervations to assign")
    else:
        cdf /= cdf[-1]
    ...
    if np.count_nonzero(unassigned_mask) > 0:
        print("Unmatched target rows:")
        print(df_target[unassigned_mask][columns].drop_duplicates())
...
def execute(context):
    ...
    df_source = pd.merge(df_source_persons, df_source_households, how='left', on='household_id')
    ...
    # TODO: Debug only
    print("Checking overlap between target and source for matching attributes...")
    source_matches_target = True
    for col in columns:
        s_vals = set(df_source[col].unique())
        t_vals = set(df_target[col].unique())
        if not (len(s_vals) == len(t_vals) == len(s_vals & t_vals)):
            source_matches_target = False
            print(f"{col}: source={len(s_vals)}, target={len(t_vals)}, overlap={len(s_vals & t_vals)}")
            print("  source-only:", s_vals - t_vals)
            print("  target-only:", t_vals - s_vals)
    assert source_matches_target
