...
def execute(context):
    ...
    colnames = ["province", "municipality", "census_section", "sex", "age", "year", "count"]
    ...
    # Clean
    population_df["municipality"] = population_df["municipality"].str[:5]
    population_df["census_section"] = population_df["census_section"].str[-5:]
    population_df.dropna(inplace=True)
    population_df["count"] = population_df["count"].astype("int64")
    ...
    condition = population_df["age"].str.startswith("70") 
    population_df.loc[condition, "age"] = 70 # sets category 70 and more years
