...
The data is provided at different aggregation levels and is used
to extrapolate age distributions down to census-section level.
"""
...
def extrapolate_age_group(context, census_path, municipality_path, province_path):
    """
    Extrapolate age distributions at census-section level.

    Steps:
    1) Load census-section population by sex
    2) Load municipality-level population by sex and age
    3) Use municipality age proportions to extrapolate census sections (municipalities ≥ 500)
    4) Use province-level age proportions for municipalities < 500
    """

    # ========== Load municipality-level data (>= 500 inhabitants) ==========

    FILE_PATH = "{}/{}".format(context.config("data_path"),context.config(municipality_path))
    ...
    # Keep only Seville province municipalities
    ...
    # Remove total rows (variable name differs across datasets)
    mun_df = mun_df[~(mun_df["shared_variable"].str.contains("otal"))]


    # ========== Load census-section-level data ==========

    FILE_PATH = "{}/{}".format(context.config("data_path"),context.config(census_path))
    print(f"Loading census data from {FILE_PATH}")
    ...
    # NOTE: the dataset for municipalities under 500 are only by sex (same as census section), 
    #       so no need to load them as well
    ...
    # ========== Province-level data (used for municipalities < 500) ==========
    ...
    # province total df
    FILE_PATH = "{}/{}".format(context.config("data_path"),context.config(province_path))
    print(f"Loading census data from {FILE_PATH}")
    province_df = pd.read_csv(FILE_PATH, sep="\t", dtype={"Total": str})
    # Present columns: National Total;Autonomous Communities and Cities;Provinces;Employment;Age;Sex;Periodo;Total;
    ...
    # ========== Extrapolation for municipalities under 500 inhabitants ==========

    # Compute age distribution by sex at province level
    ...
    # Residual population = province total - municipalities >= 500
    ...
    # Apply age proportions to census sections in small municipalities
    ...
    # ========== Extrapolation for municipalities >= 500 inhabitants ==========

    # Compute municipality-level age proportions
    ...
    # Apply municipality age distribution to census sections
...
def execute(context):
    ...
    # Average population estimates across the three different variables
