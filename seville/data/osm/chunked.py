import seville.data.osm.osmconvert
import os
...
def configure(context):
    ...
    context.stage("seville.data.osm.osmconvert")

    context.config("processes")

    context.config("data_path")
    context.config("osm_path_seville", "osm/andalucia-latest.osm.pbf")

def process_municipality(context, zone_id):
    input_path = context.data("input_path")
    local_path = context.data("local_path")

    seville.data.osm.osmconvert.run(context, [input_path,
...
def execute(context):
    ...
    with context.progress(label = "Chunking OSM data ...", total = len(df_zones)) as progress:
        with context.parallel({
            "input_path": os.path.abspath("{}/{}".format(context.config("data_path"), context.config("osm_path_seville"))),
...
def validate(context):
    return os.path.getsize("{}/{}".format(context.config("data_path"), context.config("osm_path_seville")))