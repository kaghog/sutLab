...
"""
This stage loads the raw data of the MiD german ENTD (Mobilität in Deutschland) 2023 survey and converts it to the format used by the HTS.
"""
...
PERSONEN_TCM_INDIVIDU_COLUMN = [
    ...
    "HP_SEX",  # Sex
    "BLAND",  # Region code (Bundesland)
    "P_TAET",  # Activity status include employment and student status)
]
...
def execute(context):
    personen_individu_all = pd.read_csv(
        f"{context.config('data_path')}/mid{mid_year}/MiD{mid_year}_Personen.csv",
        encoding="latin1",
        dtype={"DEP": str},
    )

    hausehold_menage_all = pd.read_csv(
        f"{context.config('data_path')}/mid{mid_year}/MiD{mid_year}_Haushalte.csv",
        encoding="latin1",
        dtype={"DEP": str},
    )

    travel_deploc_all = pd.read_csv(
        f"{context.config('data_path')}/mid{mid_year}/MiD{mid_year}_Wege.csv",
        encoding="latin1",
        dtype={"DEP": str},
    )

    # Filter where BLAND == 3 (Niedersachsen, Lower Saxony)
    ...
    df_menage = hausehold_menage.rename(
        columns={
            "H_ID": "idENT_MEN",
            "H_GEW": "PONDV1",
    ...
    df_tcm_menage = hausehold_tcm_menage.rename(
        columns={
            "H_GR": "NPERS",
            "H_GEW": "PONDV1",
    ...
    personen_individu["P_FS_PKW"] = np.where(
        personen_individu["P_FS_PKW"].isin([1]), 1, 2
    )

    personen_individu["P_FKARTE"] = np.where(
        personen_individu["P_FKARTE"].isin([1, 2, 3, 4, 5, 6]), 1, 2
    )

    personen_individu["P_FS_MOT"] = np.where(
        personen_individu["P_FS_MOT"].isin([1]), 1, 2
    )

    df_individu = personen_individu.rename(
        columns={
            "HP_ID": "IDENT_IND",
            "H_ID": "idENT_MEN",
    ...
    personen_tcm_individu["ETUDES"] = np.where(
        personen_tcm_individu["P_TAET"].isin([8, 9, 10]), 1, 2
    )
    ...
    personen_tcm_individu["SITUA"] = personen_tcm_individu["P_TAET"].replace(
        {
            1: 1,  # Employee, worker (including temporary/professional soldier)
            2: 1,  # Civil servant
            3: 1,  # Self-employed, freelancer
            4: 1,  # Marginal employment (including €520 jobs)
            5: 6,  # Employed, but currently on parental leave, care leave, partial retirement, etc.
            6: 6,  # Unpaid family member helping out in the family business
            7: 7,  # In voluntary military service / federal voluntary service (including social, ecological, or cultural year)
            8: 3,  # In training
            9: 3,  # Schoolchild, including preschool
            10: 3,  # Student
            11: 5,  # Retired, pensioner, including early retirement
            12: 4,  # Unemployed
            13: 6,  # Housewife/househusband
            14: 7,  # Permanently unable to work
            15: 7,  # Child, cared for at home
            16: 7,  # Child, in kindergarten or other day care
            17: 7,  # Other
    ...
    personen_tcm_individu["HP_SEX"] = personen_tcm_individu["HP_SEX"].replace(
        {
            1: 1,
            2: 2,
            3: 1,  # set "diverse" as male
            9: 1,  # Set unknown as male
        }
    )

    personen_tcm_individu_draft = personen_tcm_individu.rename(
        columns={
            "HP_ALTER": "AGE",
            "HP_ID": "IDENT_IND",
            "H_ID": "IDENT_MEN",
...
def validate(context):
    for name in ("MiD2023_Personen.csv", "MiD2023_Haushalte.csv", "MiD2023_Wege.csv"):
        if not os.path.exists("%s/mid2023/%s" % (context.config("data_path"), name)):
            raise RuntimeError("File missing from ENTD: %s" % name)

    return [
        os.path.getsize(
            "%s/mid2023/MiD2023_Personen.csv" % context.config("data_path")
        ),
        os.path.getsize(
            "%s/mid2023/MiD2023_Haushalte.csv" % context.config("data_path")
        ),
        os.path.getsize("%s/mid2023/MiD2023_Wege.csv" % context.config("data_path")),
    ]
