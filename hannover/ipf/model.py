...
def execute(context):
    ...
    for age_class in combined_age_classes:
        population_age_mapping[age_class] = population_age_classes[
            np.count_nonzero(population_age_upper <= age_class)
        ]
        employment_age_mapping[age_class] = employment_age_classes[
            np.count_nonzero(employment_age_upper <= age_class)
        ]
        license_age_mapping[age_class] = license_age_classes[
            np.count_nonzero(license_age_upper <= age_class)
        ]
    ...
    unique_kreis = np.sort(df_employment["kreis_index"].unique())
    ...
    # Attach kreis and departement indices
    df_spatial = df_population[
        ["commune_index", "kreis_index", "departement_index"]
    ].drop_duplicates()
    df_model["kreis_index"] = df_model["commune_index"].replace(
        dict(zip(df_spatial["commune_index"], df_spatial["kreis_index"]))
    )
    df_model["departement_index"] = df_model["commune_index"].replace(
        dict(zip(df_spatial["commune_index"], df_spatial["departement_index"]))
    )

    # Attach individual age classes
    df_model["age_class_population"] = df_model["combined_age_class"].replace(
        population_age_mapping
    )
    df_model["age_class_employment"] = df_model["combined_age_class"].replace(
        employment_age_mapping
    )
    df_model["age_class_license"] = df_model["combined_age_class"].replace(
        license_age_mapping
    )
    ...
    combinations = list(
        itertools.product(unique_communes, unique_sexes, population_age_classes)
    )
    for combination in context.progress(
        combinations, total=len(combinations), label="Generating population constraints"
    ):
    ...
    # Employment constraints at kreis level
    combinations = list(
        itertools.product(unique_kreis, unique_sexes, employment_age_classes)
    )
    for combination in context.progress(
        combinations, total=len(combinations), label="Generating employment constraints"
    ):
        f_reference = df_employment["kreis_index"] == combination[0]
        f_reference &= df_employment["sex"] == combination[1]
        f_reference &= df_employment["age_class"] == combination[2]

        f_model = df_model["kreis_index"] == combination[0]
        f_model &= df_model["sex"] == combination[1]
    ...
    for combination in context.progress(
        combinations, total=len(combinations), label="Generating license constraints"
    ):
    ...
    for kreis_index in context.progress(
        unique_kreis,
        total=len(unique_kreis),
        label="Generating license constraints per Kreis",
    ):
        f_reference = df_licenses_kreis["kreis_index"] == kreis_index

        f_model = df_model["kreis_index"] == kreis_index
        f_model &= df_model["license"]  # Only select people with license!
        selectors.append(f_model)
    ...
    while iteration < 1000:
        ...
        print(
            ...
            iteration,
            "factors:",
            len(iteration_factors),
            "mean:",
            np.mean(iteration_factors),
            "min:",
            np.min(iteration_factors),
            "max:",
            np.max(iteration_factors),
        )
    ...
    df_model["sex"] = (
        df_model["sex"].replace({1: "male", 2: "female"}).astype("category")
    )

    # Add identifiers
    df_model = pd.merge(
        df_model,
        df_population[["commune_index", "commune_id"]].drop_duplicates(),
        on="commune_index",
        how="left",
    )
    ...
    df_model = pd.merge(
        df_model,
        df_population[["kreis_index", "kreis_code"]].drop_duplicates(),
        on="kreis_index",
        how="left",
    )
    assert np.count_nonzero(df_model["kreis_code"].isna()) == 0

    df_model = pd.merge(
        df_model,
        df_population[["departement_index", "departement_id"]].drop_duplicates(),
        on="departement_index",
        how="left",
    )
