...
def configure(context):
    context.stage("hannover.ipu.prepare")
    context.stage("hannover.data.hts.entd.filtered")
...
def execute(context):
    census_targets_by_dept = context.stage("hannover.ipu.prepare")
    df_households_hts, df_persons_hts, df_trips_hts = context.stage(
        "hannover.data.hts.entd.filtered"
    )
    ...
    # Map HTS age to census age_class bins [0, 6, 15, 18, 24, 30, 45, 65, 80]
    hts_df["age_class"] = pd.cut(
        hts_df["age"],
        bins=[0, 6, 15, 18, 24, 30, 45, 65, 80, np.inf],
        labels=[0, 6, 15, 18, 24, 30, 45, 65, 80],
        right=False,
    ).astype(int)

    hts_df["household_size_capped"] = hts_df["household_size"].clip(upper=6)
