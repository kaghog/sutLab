...
def configure(context):
    ...
    # Hannover-specific constraints for PT and license
...
"""
Hannover enriched stage
- Starts from the generic enriched population
- Calibrates PT subscription against Hannover HTS-derived constraints
- Enforces PT subscription = 0 for ages <6
- Calibrates driving license against Hannover HTS-derived constraints
...
def execute(context):
    ...
    constraints_pt = context.stage("hannover.data.pt.constraints")["pt_subscription_constraints"]
    constraints_lic = context.stage("hannover.data.license.constraints")["license_constraints"]
