...
def plot_comparison_bar(
    ...
    census=None,
    actual=None,
    lablist=["HTS", "Synthetic", "Simulation", "Census"],
    t=15,
    figsize=[12, 7],
    dpi=300,
    w=0.25,
    ...
    datasets = {
        ...
        "synthetic": synthetic[:top]
        if synthetic is not None and top is not None
        else synthetic,
        "simulation": simulation[:top]
        if simulation is not None and top is not None
        else simulation,
    ...
    # Filter out None datasets first
    active_datasets = {k: v for k, v in datasets.items() if v is not None}

    # Build legend_labels by matching active datasets with provided labels
    # The lablist should correspond to active datasets in order
    default_labels = {
        "hts": "HTS",
        "synthetic": "Synthetic",
    ...
    for i, key in enumerate(active_keys):
        if i < len(lablist):
            legend_labels[key] = lablist[i]
        else:
            legend_labels[key] = default_labels[key]
...
def compute_counts(
    series, weights=None, categories=None, normalize=True, to_percent=True
):
    """
...
def plot_distribution_differences(
    ...
    diff_census=None,
    diff_actual=None,
    lablist=["Synthetic vs HTS", "Synthetic vs Census"],
    t=15,
    figsize=[12, 7],
    dpi=300,
    w=0.35,
...
def plot_horizontal_comparison(
    ...
    for j in range(n):
        ...
        offsets = np.linspace(
            -bar_height / 2 + sub_h / 2, bar_height / 2 - sub_h / 2, k_valid
        )

        # Draw bars for this label
        for idx, (val, color, label) in enumerate(
            zip(valid_values, valid_colors, valid_labels)
        ):
...
def plot_comparison_hist_purpose(
    context,
    title,
    actual_df,
    synthetic_df,
    ...
    fig, axes = plt.subplots(
        nrows=actual_rows, ncols=cols, figsize=(5 * cols, 3 * actual_rows)
    )
    ...
    for r in range(actual_rows):
        for c in range(cols):
            if idx < len(modelist):
                ...
                x = synthetic_df[synthetic_df["following_purpose"] == purpose][
                    "crowfly_distance"
                ]
                y = actual_df[actual_df["purpose"] == purpose][
                    ["crowfly_distance", "weight_person"]
                ]
...
def plot_comparison_hist_mode(
    ...
    for r in range(actual_rows):
        for c in range(cols):
            if idx < len(modelist):
                x = synthetic_df[synthetic_df["mode"] == modelist[idx]][
                    "crowfly_distance"
                ]
                y = actual_df[actual_df["mode"] == modelist[idx]][
                    ["crowfly_distance", "weight_person"]
                ]
...
def plot_comparison_cdf_purpose(
    context, title, actual_df, synthetic_df, dpi=300, cols=3, rows=2
):
    import pandas as pd
    ...
    if actual_df is not None:
        purpose_col = (
            "purpose" if "purpose" in actual_df.columns else "following_purpose"
        )
        ...
        for c in range(cols):
            if idx < len(modelist):
                ...
                if actual_df is not None and len(actual_df) > 0:
                    ...
                    purpose_col = (
                        "purpose"
                        if "purpose" in actual_df.columns
                        else "following_purpose"
                    )
                    if purpose_col in actual_df.columns:
                        y = actual_df[actual_df[purpose_col] == purpose][
                            ["crowfly_distance", "weight_person"]
                        ]
                        if len(y) == 0:  # No HTS data for this purpose
                            y = pd.DataFrame(
                                columns=["crowfly_distance", "weight_person"]
                            )
                    else:
...
def plot_mode_share(context, title, df_syn, df2, amdf2, dpi=300):
    modelist = list(
        zip(
            np.sort(df_syn["mode"].unique()),
            ["car", "car_passenger", "pt", "taxi", "walk"],
        )
    )
    plt.rcParams["figure.dpi"] = dpi
    y1 = []
    ...
    for mode, mode_cat in modelist:
        ...
        y2.append(
            amdf2[amdf2["mode"] == mode_cat]["weight_person"].sum()
            / amdf2["weight_person"].sum()
        )
...
def add_synthetic_cdf(axes, r, c, act, x, label="Synthetic"):
    ...
    axes[r, c].plot(
        x_data[x_sorted], x_cdf, label=label, color=COLOR_ACTUAL_HTS, linewidth=2
    )
...
def plot_synthetic_cdf_purpose(context, title, synthetic_df, dpi=300, cols=3, rows=2):
    ...
    fig.tight_layout()
    plt.savefig(
        f"{context.config('analysis_path')}/{title}", bbox_inches="tight", dpi=dpi
    )
    plt.close()
...
def add_threeway_hist(
    axes, r, c, act, x_hts, x_syn1, x_syn2, bins, label_syn1="CARLA", label_syn2="Hoerl"
):
    """Add a three-way histogram comparison to a subplot."""
    # HTS histogram (weighted)
    if x_hts is not None and len(x_hts) > 0:
        axes[r, c].hist(
            x_hts["crowfly_distance"],
            bins,
            weights=x_hts["weight_person"],
            alpha=0.4,
            density=True,
            color=COLOR_ACTUAL_HTS,
        )
...
def add_threeway_cdf(
    axes, r, c, act, x_hts, x_syn1, x_syn2, label_syn1="CARLA", label_syn2="Hoerl"
):
...
def plot_threeway_hist_purpose(
    context,
    title,
    actual_df,
    synthetic_df1,
    ...
    for r in range(actual_rows):
        for c in range(cols):
            if idx < len(modelist):
                purpose = modelist[idx]
                x_syn1 = synthetic_df1[synthetic_df1["following_purpose"] == purpose][
                    "crowfly_distance"
                ]
                x_syn2 = synthetic_df2[synthetic_df2["following_purpose"] == purpose][
                    "crowfly_distance"
                ]
                y_hts = None
                if actual_df is not None:
                    y_hts = actual_df[actual_df["purpose"] == purpose][
                        ["crowfly_distance", "weight_person"]
                    ]
                axes = add_threeway_hist(
                    axes,
                    r,
                    c,
                    purpose,
                    ...
                    x_syn2,
                    bins,
                    label_syn1=label_syn1,
                    label_syn2=label_syn2,
                )
                idx += 1
            else:
                axes[r, c].set_visible(False)
...
def plot_threeway_hist_mode(
    ...
    for r in range(actual_rows):
        for c in range(cols):
            if idx < len(modelist):
                mode = modelist[idx]
                x_syn1 = synthetic_df1[synthetic_df1["mode"] == mode][
                    "crowfly_distance"
                ]
                x_syn2 = synthetic_df2[synthetic_df2["mode"] == mode][
                    "crowfly_distance"
                ]
                y_hts = None
                if actual_df is not None:
                    y_hts = actual_df[actual_df["mode"] == mode][
                        ["crowfly_distance", "weight_person"]
                    ]
                axes = add_threeway_hist(
                    axes,
