...
warnings.filterwarnings(
    "ignore", message=r".*edges kwarg.*node_link_data.*", category=FutureWarning
)
...
def configure(context):
    ...
    context.config(
        "specific_weekend_scenario", "all"
    )  # options are "all", "saturday", "sunday"
    context.config(
        "specific_day_scenario", "avgworkday"
    )  # options can be any of the days of the week or "avgworkday"
...
def import_data_synthetic(context, population_selector=None, custom_output_path=None):
    ...
    """
    output_path = (
        custom_output_path if custom_output_path else context.config("output_path")
    )
    ...
    df_persons["is_active"] = (
        df_persons["person_id"].isin(df_trips["person_id"]).astype(bool)
    )
    ...
    if population_selector:
        if "age_selector" in population_selector.keys():
            ...
            df_persons = df_persons[
                (df_persons["age"] <= age_max) & (df_persons["age"] >= age_min)
            ]
            df_trips = df_trips[
                (df_trips["age"] <= age_max) & (df_trips["age"] >= age_min)
            ]
            df_persons_no_trip = df_persons_no_trip[
                (df_persons_no_trip["age"] <= age_max)
                & (df_persons_no_trip["age"] >= age_min)
            ]
            print(
                "INFO excluding agents NOT between the age of ",
                age_min,
                " and ",
                age_max,
            )
        ...
        if "canton_selector" in population_selector.keys():
            ...
            df_persons_no_trip = df_persons_no_trip[
                df_persons_no_trip["canton_id"].isin(cantons)
            ]
...
def import_data_actual(context, population_selector=None):
    ...
    df_act_households["number_of_vehicles"] = pd.to_numeric(
        df_act_households["number_of_vehicles"], errors="coerce"
    ).fillna(0)
    ...
    df_act_persons["car_availability"] = df_act_persons["car_availability"].fillna(
        False
    )

    df_act_persons.rename(columns={"person_weight": "weight_person"}, inplace=True)
    df_px = df_act_persons[
        [
            "person_id",
            "weight_person",
            "employed",
    ...
    df_persons_no_trip = df_act_persons[
        np.logical_not(df_act_persons["person_id"].isin(t_id))
    ]
    df_persons_no_trip = df_persons_no_trip.set_index(["person_id"])
...
def household_size_distribution(context, df_syn_persons, df_census_hh, suffix=None):
    # Calculate census household size distribution
    census_sizes = {}
    census_cols_map = {
        "households_1_person": "1",
        "households_2_persons": "2",
        "households_3_persons": "3",
        "households_4_persons": "4",
        "households_5_persons": "5",
        "households_6plus_persons": "6+",
    }

    for col, label in census_cols_map.items():
        if col in df_census_hh.columns:
            census_sizes[label] = df_census_hh[col].sum()

    # Convert to percentages
    total_census_hh = sum(census_sizes.values())
    census_pct = {k: (v / total_census_hh * 100.0) for k, v in census_sizes.items()}

    # Calculate synthetic household sizes from passed dataframe
    if "household_id" not in df_syn_persons.columns:
        return

    syn_hh_sizes = df_syn_persons.groupby("household_id").size()
    total_syn_hh = len(syn_hh_sizes)

    # Create distribution with same categories as census
    syn_sizes = {}
    syn_sizes["1"] = (syn_hh_sizes == 1).sum()
    syn_sizes["2"] = (syn_hh_sizes == 2).sum()
    syn_sizes["3"] = (syn_hh_sizes == 3).sum()
    syn_sizes["4"] = (syn_hh_sizes == 4).sum()
    syn_sizes["5"] = (syn_hh_sizes == 5).sum()
    syn_sizes["6+"] = (syn_hh_sizes >= 6).sum()

    # Convert to percentages
    syn_pct = {k: (v / total_syn_hh * 100.0) for k, v in syn_sizes.items()}

    # Prepare data for plotting
    labels = ["1", "2", "3", "4", "5", "6+"]
    census_vals = [census_pct.get(l, 0.0) for l in labels]
    syn_vals = [syn_pct.get(l, 0.0) for l in labels]

    # Plot using existing comparison bar function
    imtitle = "household_size_distribution"
    plottitle = "Household Size Distribution Comparison"
    if suffix:
        imtitle += f"_{suffix}"
        plottitle += f" - {suffix}"
    imtitle += ".png"
...
def aux_data_frame(
    df_act_trips, df_syn_trips, df_act_persons, df_syn_persons, population_selector=None
):
    ...
    df_act_reset = (
        df_act_trips.reset_index()
    )  # brings index name 'person_id' as a column
    if (
        "weight_person" not in df_act_reset.columns
        and "weight_person" in df_act_persons.columns
    ):
    ...
    df_aux_act = pd.DataFrame(
        {
            ...
            "chain": "home-"
            + df_act_reset.groupby("person_id")["following_purpose"].apply(
                lambda x: "-".join(x)
            ),
        }
    ...
    df_aux_syn = pd.DataFrame(
        {
            ...
            "weights": 1,
            "chain": "home-"
            + df_syn_trips.groupby("person_id")["following_purpose"].apply(
                lambda x: "-".join(x)
            ),
        }
    )
...
def activity_counts_comparison(context, all_CC, suffix=None):
    ...
    for k in range(min(8, np.max(list(counts_dic.keys())))):
        ...
        if k == 7:
            ...
            l = str(int(k))
        counts.loc[k] = pd.Series(
            {"number": l, "synthetic Count": v[0], "actual Count": v[1]}
        )

    # Get percentages, prepare for plotting
    counts["synthetic Count"] = (
        counts["synthetic Count"] / counts["synthetic Count"].sum() * 100
    )
    ...
    myplottools.plot_comparison_bar(
        ...
        plottitle=title_plot,
        ylabel="Percentage",
        xlabel="Number of activities in the activity chain",
        lab=counts["number"],
        actual=counts["actual Count"],
...
def activity_counts_per_purpose(context, all_CC, suffix=None):
    ...
    for k, v in counts_dic.items():
        counts.loc[k] = pd.Series(
            {"number": k, "synthetic Count": v[0], "actual Count": v[1]}
        )
    ...
    myplottools.plot_comparison_bar(
        ...
        plottitle=title_plot,
        ylabel="Percentage",
        xlabel="Activities with the same purpose in the activity chain",
        lab=counts["number"],
        actual=counts["actual Count"],
        synthetic=counts["synthetic Count"],
        t=20,
...
def demographics_comparison(
    ...
    cols_act = [
        ...
        if c in df_act_persons.columns
    ]
    cols_syn = [
        c
        for c in [
        ...
        if c in df_syn_persons.columns
    ]
    df_act_persons = (
        df_act_persons[cols_act].drop_duplicates(subset=["person_id"]).copy()
    )
    df_syn_persons = (
        df_syn_persons[cols_syn].drop_duplicates(subset=["person_id"]).copy()
    )
    ...
    act_counts = myplottools.compute_counts(
        df_act_persons["age_bin"],
        weights=df_act_persons["weight_person"],
        categories=labels,
    )
    syn_counts = myplottools.compute_counts(
        df_syn_persons["age_bin"], weights=None, categories=labels
    )
    ...
    if df_census is None:
        print(
            "[DEBUG] Census is None in demographics_comparison; plotting will be HTS vs SYN only."
        )
        ...
        df_debug = pd.DataFrame(
            {
                ...
                "gap_syn_minus_census": (syn_counts - census_counts)
                .reindex(ordered_labels)
                .values,
                "gap_hts_minus_census": (act_counts - census_counts)
                .reindex(ordered_labels)
                .values,
            }
        )
    ...
    act_counts = myplottools.compute_counts(
        df_act_persons_local["employment_status"],
        weights=df_act_persons_local["weight_person"],
    ...
    syn_counts = myplottools.compute_counts(
        syn_employment, weights=None, categories=act_counts.index.tolist()
    )
    ...
    act_license_labels = myplottools.map_bool_to_labels(
        df_act_persons.get("has_license"), yes_label="Yes", no_label="No"
    )
    act_counts = myplottools.compute_counts(
        act_license_labels,
        weights=df_act_persons.get("weight_person"),
        categories=["No", "Yes"],
    )  # fixed order

    syn_license_labels = myplottools.map_bool_to_labels(
        df_syn_persons.get("has_driving_license"), yes_label="Yes", no_label="No"
    )
    syn_counts = myplottools.compute_counts(
        syn_license_labels, weights=None, categories=act_counts.index.tolist()
    )
    syn_counts = myplottools.align_series(act_counts, syn_counts)
    ...
    act_pt_labels = myplottools.map_bool_to_labels(
        df_act_persons.get("has_pt_subscription"), yes_label="Yes", no_label="No"
    )
    act_counts = myplottools.compute_counts(
        act_pt_labels,
        weights=df_act_persons.get("weight_person"),
    ...
    syn_pt_labels = myplottools.map_bool_to_labels(
        df_syn_persons.get("has_pt_subscription"), yes_label="Yes", no_label="No"
    )
    syn_counts = myplottools.compute_counts(
        syn_pt_labels, weights=None, categories=act_counts.index.tolist()
    )
    syn_counts = myplottools.align_series(act_counts, syn_counts)
...
def summary_horizontal(
    ...
    - syn vs hts: employment, driving license, pt subscription

    All metrics are expressed as percentages of persons (HTS/Census weighted; Synthetic unweighted).
    Employment uses HTS (real survey data) instead of census (imputed).
    ...
    if df_census is not None and {"age_class", "weight"}.issubset(df_census.columns):
        ...
        cen_age_pct = (
            (cen_age_raw / cen_age_raw.sum() * 100.0).reindex(age_labels).fillna(0)
        )
    ...
    if syn_sex_series is not None:
        syn_sex_norm = (
            syn_sex_series.replace({1: "male", 2: "female", 0: "male"})
            .astype(str)
            .str.lower()
        )
    ...
    # HTS employment (real survey data, not imputed)
    act_emp_series = df_act_persons.get("employed")
    if act_emp_series is not None:
        act_emp_lab = act_emp_series.replace({False: "Unemployed", True: "Employed"})
        _wcol_emp = (
            "weight_person"
            if "weight_person" in df_act_persons.columns
            else (
                "person_weight" if "person_weight" in df_act_persons.columns else None
            )
        )
        _wser_emp = df_act_persons[_wcol_emp] if _wcol_emp is not None else None
        act_emp_pct = myplottools.compute_counts(
            act_emp_lab, weights=_wser_emp, categories=emp_labels
        )
    else:
        act_emp_pct = _pd.Series([float("nan")] * 2, index=emp_labels)
    ...
    syn_lic_lab = (
        _pd.Series(syn_lic_series).replace(
            {False: "Driving license No", True: "Driving license Yes"}
        )
        if syn_lic_series is not None
        else None
    )
    syn_lic_pct = (
        myplottools.compute_counts(syn_lic_lab, categories=lic_labels)
        if syn_lic_lab is not None
        else _pd.Series([float("nan")] * 2, index=lic_labels)
    )

    act_lic_series = df_act_persons.get("has_license")
    act_lic_lab = (
        _pd.Series(act_lic_series).replace(
            {False: "Driving license No", True: "Driving license Yes"}
        )
        if act_lic_series is not None
        else None
    )
    _wcol = (
        "weight_person"
        if "weight_person" in df_act_persons.columns
        else ("person_weight" if "person_weight" in df_act_persons.columns else None)
    )
    _wser = df_act_persons[_wcol] if _wcol is not None else None
    act_lic_pct = (
        myplottools.compute_counts(act_lic_lab, weights=_wser, categories=lic_labels)
        if act_lic_lab is not None
        else _pd.Series([float("nan")] * 2, index=lic_labels)
    )
    ...
    syn_pts_lab = (
        _pd.Series(syn_pts_series).replace(
            {False: "PT Subscription No", True: "PT Subscription Yes"}
        )
        if syn_pts_series is not None
        else None
    )
    syn_pts_pct = (
        myplottools.compute_counts(syn_pts_lab, categories=pts_labels)
        if syn_pts_lab is not None
        else _pd.Series([float("nan")] * 2, index=pts_labels)
    )

    act_pts_series = df_act_persons.get("has_pt_subscription")
    act_pts_lab = (
        _pd.Series(act_pts_series).replace(
            {False: "PT Subscription No", True: "PT Subscription Yes"}
        )
        if act_pts_series is not None
        else None
    )
    _wcol2 = (
        "weight_person"
    ...
    act_pts_pct = (
        myplottools.compute_counts(act_pts_lab, weights=_wser2, categories=pts_labels)
        if act_pts_lab is not None
        else _pd.Series([float("nan")] * 2, index=pts_labels)
    )
...
def compute_distances_actual(df_act, threshold=25):
    ...
    df_act["crowfly_distance"] = (
        df_act["routed_distance"] / 1000.0
...
def compare_dist_from_home(
    context, df_syn, df_act, target_purpose="education", suffix=None
):
    if "origin_purpose" not in df_act.columns:
        df_act.loc[:, "origin_purpose"] = df_act["preceding_purpose"]

    # select candidates
    filter_home_prim_syn = (df_syn["following_purpose"] == target_purpose) & (
        df_syn["preceding_purpose"] == "home"
    )
    filter_home_prim_act = (df_act["following_purpose"] == target_purpose) & (
        df_act["origin_purpose"] == "home"
    )
    filter_prim_home_syn = (df_syn["following_purpose"] == "home") & (
        df_syn["preceding_purpose"] == target_purpose
    )
    filter_prim_home_act = (df_act["following_purpose"] == "home") & (
        df_act["origin_purpose"] == target_purpose
    )

    df_syn_educ = df_syn[filter_home_prim_syn | filter_prim_home_syn].drop_duplicates(
        subset=["person_id"]
    )
    df_act_educ = df_act[filter_prim_home_act | filter_home_prim_act].drop_duplicates(
        subset=["person_id"]
    )
    ...
    dic_syn = {
        "person_id": pers_educ_syn,
        "dist_home_educ": [0 for i in range(len(pers_educ_syn))],
    }
    dic_act = {
        "person_id": pers_educ_act,
        "weight_person": [0 for i in range(len(pers_educ_act))],
        "dist_home_educ": [0 for i in range(len(pers_educ_act))],
    }
    ...
    for i in range(len(pers_educ_act)):
        ...
        for _, row in df_pers.iterrows():
            ...
            if have_home and have_educ:
                break
        if have_home and have_educ:
            dist_val = 0.001 * float(
                np.sqrt(((home_x - educ_x) ** 2 + (home_y - educ_y) ** 2))
            )
...
def all_the_plot_distances(
    ...
    myplottools.plot_comparison_cdf_purpose(
        context, dpc_title, df_act_dist, df_syn_dist, dpi=300, cols=3, rows=2
    )
    ...
    if "mode" in df_syn_dist.columns and "mode" in df_act_dist.columns:
        ...
        myplottools.plot_comparison_cdf_mode(
            context, dmc_title, df_act_dist, df_syn_dist, dpi=300, cols=3, rows=2
        )
    ...
    if df_syn_dist_alt is not None:
        print(
            f"INFO Algorithm comparison mode: generating three-way plots (HTS vs CARLA vs {alt_label})"
        )
        ...
        print(
            f"SUCCESS: Created three-way comparison plots: {dph_title_3way}, {dpc_title_3way}"
        )

        # Three-way comparison for mode distances if available
        if (
            "mode" in df_syn_dist.columns
            and "mode" in df_act_dist.columns
            and "mode" in df_syn_dist_alt.columns
        ):
            print(
                f"INFO generating three-way distance plots by mode (HTS vs CARLA vs {alt_label})"
            )
            ...
            print(
                f"SUCCESS: Created three-way mode comparison plots: {dmh_title_3way}, {dmc_title_3way}"
            )
    else:
        print(
            "INFO skipping mode distance plots - mode data not available in both datasets"
        )
...
def generate_plots(
    ...
    if hts_available and len(df_aux_act) > 0:
        act_CC = (
            df_aux_act.groupby("chain")["weight_person"].sum().reset_index(name="count")
        )
    else:
        # Create empty HTS data for compatibility
        act_CC = pd.DataFrame(columns=["chain", "weight_person"])
        act_CC = (
            act_CC.groupby("chain")["weight_person"].sum().reset_index(name="count")
        )
    ...
    syn_CC.loc[len(syn_CC) + 1] = pd.Series(
        {"Chain": "home", "synthetic Count": df_syn_no_trip.shape[0]}
    )
    ...
    act_CC.loc[len(act_CC) + 1] = pd.Series(
        {"Chain": "home", "actual Count": act_no_trip_weight}
    )
    ...
    df_census_hh = context.stage("hannover.data.census.households")
    household_size_distribution(context, df_syn_persons, df_census_hh, suffix)
    ...
    try:
        ...
        if hts_available:
            ...
            print(
                f"INFO distances computed - Synthetic: {df_syn_dist.shape}, HTS: {df_act_dist.shape}"
            )
        else:
            print("INFO HTS data not available - synthetic distances only")
            df_act_dist = None
            print(
                f"INFO distances computed - Synthetic: {df_syn_dist.shape}, HTS: None"
            )
        ...
        if hts_available and df_act_dist is not None:
            ...
            if "weight_person" not in df_act_dist_with_weight.columns:
                ...
                df_act_dist_with_weight = df_act_dist_with_weight.merge(
                    df_act_persons[["person_id", "weight_person"]].drop_duplicates(
                        "person_id"
                    ),
                    on="person_id",
                    how="left",
                )
            df_act_dist_with_weight["weighted_distance"] = (
                df_act_dist_with_weight["weight_person"]
                * df_act_dist_with_weight["crowfly_distance"]
            )

            # Calculate weighted mean distances by purpose for HTS
            act_weighted_sum = df_act_dist_with_weight.groupby("following_purpose")[
                "weighted_distance"
            ].sum()
            act_weight_sum = df_act_dist_with_weight.groupby("following_purpose")[
                "weight_person"
            ].sum()
        ...
        syn_mean_distances = df_syn_dist.groupby("following_purpose")[
            "crowfly_distance"
        ].mean()

        # Align purposes between datasets
        all_purposes = list(
            set(act_mean_distances.index.tolist() + syn_mean_distances.index.tolist())
        )
        ...
        if (
            df_act_dist is not None
            and df_act_dist_with_weight is not None
            and len(df_act_dist) > 0
        ):
            ...
            if (
                ...
                df_hts_for_plotting["purpose"] = df_hts_for_plotting[
                    "following_purpose"
                ]
            print(
                f"INFO HTS data available for comparison: {len(df_hts_for_plotting)} trips"
            )
        else:
        ...
        try:
            ...
            myplottools.plot_comparison_cdf_purpose(
                context, cdf_title, df_hts_for_plotting, df_syn_dist, dpi=300
            )
        ...
        try:
            if df_hts_for_plotting is not None:
                all_the_plot_distances(
                    context,
                    df_hts_for_plotting,
                    df_syn_dist,
                    suffix,
                    df_syn_dist_alt=df_syn_dist_alt,
                    alt_label="Hoerl",
                )
...
def execute(context):
    ...
    if COMPARE_LOCATION_ALGORITHMS:
        ...
        try:
            ...
            df_syn_persons_alt, df_syn_trips_alt, df_syn_no_trip_alt = (
                import_data_synthetic(
                    context, None, custom_output_path=HOERL_OUTPUT_PATH
                )
            )
            print("SUCCESS: Loaded alternative algorithm data")
    ...
    for population_selector, suffix in list(zip(pop_selectors, suffixes)):
        df_syn_persons, df_syn_trips, df_syn_no_trip = import_data_synthetic(
            context, population_selector
        )
        ...
        if hts_result[0] is None:
            ...
            df_aux_act, df_aux_syn = aux_data_frame(
                df_act_trips, df_syn_trips, df_act_persons, df_syn_persons
            )
        ...
        generate_plots(
            ...
            suffix,
            df_census,
            df_syn_trips_alt=df_syn_trips_alt,
        )
