...
def execute(context):
    ...
    df_activities = pd.read_csv(filepath, encoding = "latin1", sep = ";")
    ...
    df_households = pd.read_csv(filepath, encoding = "latin1", sep = ";")
    ...
    df_activities["is_first_trip"] = df_activities["is_first"]
    df_activities["is_last_trip"] = df_activities["is_last"]
    df_activities["trip_id"] = df_activities["trip_index"]

    df_activities["start_time"] = df_activities.shift(1)["arrival_time"]
    
    df_activities.loc[df_activities["is_first"], "start_time"] = np.nan
