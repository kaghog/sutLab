...
def __plot_household(context):
    df_census = context.stage("seville.data.census.households").copy()
    df_ipu = context.stage("seville.ipu.attributed").copy()
    df_hts, _, _ = context.stage("seville.hts.entd.filtered").copy()
    ...
    df_census = df_census[household_size_cols].sum()
    df_census = df_census / df_census.sum() * 100

    df_ipu = df_ipu.groupby('household_id').size().reset_index()
    df_ipu = df_ipu.value_counts().sort_index()
    df_ipu = df_ipu / df_ipu.sum() * 100

    df_hts = df_hts.value_counts('household_size').sort_index()
    df_hts = df_hts / df_hts.sum() * 100
    ...
    plt.bar(x - w, census_w, width=w, label="Census")
    plt.bar(x,     ipu_w,    width=w, label="IPU synthetic")
    plt.bar(x + w, output_w, width=w, label="Final output")
