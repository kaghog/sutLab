...
"""
Generates a distance matrix for the asuncion's census sections.
"""
...
def execute(context):
    ...
    _, population_centroids, employment_centroids, _ = context.stage("asuncion.gravity.od_zones")

    print(population_centroids['macrozone_id'])
    print(employment_centroids['macrozone_id'])    
    assert len(population_centroids) == len(employment_centroids), f"{len(population_centroids)} == {len(employment_centroids)}"
