...
def impute_education_locations_radius(context):
    ...
    school_levels = sorted(graduation_age.items(),key=lambda x: x[1]
    )
    age_bounds = [] #[(-np.inf, 16), (17, np.inf)]
    education_types = [] #[["school"], ["university"]]
    min_year = -np.inf
    for idx, (edu_type, max_year) in enumerate(school_levels):
        education_types.append([edu_type])
        age_bounds.append((min_year, max_year))
        min_year = max_year + 1

    education_types.append(["university"])
    age_bounds.append((min_year, np.inf))
    # Group destinations into age categories
    

    
    query_sizes = [1 for i in range(len(education_types))] #[5, 5, 5]
    ...
    if n_missing > 0:
        print(f"ERROR: {n_missing}/{len(df_education_persons)} persons were not assigned education locations!")
