...
def add_person(writer, person, activities, trips, vehicles):
    ...
    writer.add_attribute(
        "bicycleAvailability",
        "java.lang.String",
        person[PERSON_FIELDS.index("bicycle_availability")],
    )
...
def execute(context):
    ...
    # Re-sort activities to match person order (by household_id, person_id, then activity_index)
    df_activities = (
        df_activities.merge(
            df_persons[["person_id", "household_id"]], on="person_id", how="left"
        )
        .sort_values(by=["household_id", "person_id", "activity_index"])
    ...
    # Re-sort trips to match person order (by household_id, person_id, then trip_index)
    df_trips = (
        df_trips.merge(
            df_persons[["person_id", "household_id"]], on="person_id", how="left"
        )
        .sort_values(by=["household_id", "person_id", "trip_index"])
        .drop(columns=["household_id"])
    )

    df_vehicles = context.stage("synthesis.vehicles.vehicles")[1]

    # Re-sort vehicles to match person order (by household_id, person_id)
    df_vehicles = (
        df_vehicles.merge(
        ...
        .sort_values(by=["household_id", "owner_id"])
        .drop(columns=["household_id", "person_id"])
    )
