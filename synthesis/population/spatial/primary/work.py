...
def prepare_work_persons(context):
    ...
    df_work_persons = df_persons[
        df_persons["person_id"].isin(df_trips_work["person_id"].unique())
    ].copy()
...
def impute_work_locations_radius(context):
    ...
    df_work_persons["home_x"] = df_work_persons["home_geometry"].apply(
        lambda geom: geom.x
    )
    df_work_persons["home_y"] = df_work_persons["home_geometry"].apply(
        lambda geom: geom.y
    )
    home_coordinates = np.vstack(
        [df_work_persons["home_x"], df_work_persons["home_y"]]
    ).T
    ...
    work_coordinates = np.vstack(
        [df_work_candidates["destination_x"], df_work_candidates["destination_y"]]
    ).T
    tree = KDTree(work_coordinates)

    # Sample distances and find candidates within radius
    indices, distances = tree.query_radius(
        home_coordinates, r=radius, return_distance=True, sort_results=True
    )
    ...
    for i, (ind, dist) in enumerate(zip(indices, distances)):
        ...
        # If enough facilities, apply donut selection
            ...
            donut_ind = ind[
                (dist >= minimum_selection_bound) & (dist <= maximum_selection_bound)
            ]
    ...
    df_work_persons["commune_id"] = df_work_candidates.iloc[chosen_indices][
        "commune_id"
    ].values
    df_work_persons["location_id"] = df_work_candidates.iloc[chosen_indices][
        "location_id"
    ].values
    df_work_persons["geometry"] = df_work_candidates.iloc[chosen_indices][
        "geometry"
    ].values

    # Calculate actual distances for validation
    df_work_persons["work_x"] = df_work_candidates.iloc[chosen_indices][
        "destination_x"
    ].values
    df_work_persons["work_y"] = df_work_candidates.iloc[chosen_indices][
        "destination_y"
    ].values
    ...
    print("INFO: Work distance statistics:")
