...
def prepare_education_persons(context):
    ...
    df_education_persons = df_persons[
        df_persons["person_id"].isin(df_trips_education["person_id"].unique())
    ].copy()
...
def prepare_education_destinations(context):
    ...
    # Note: Include fake/synthetic locations to enable longer-distance assignments
...
def prepare_radius_from_cdf(context, df_education_persons):
    distributions = context.stage(
        "synthesis.population.spatial.primary.distance_distributions"
    )
...
def impute_education_locations_radius(context):
    ...
    df_education_persons["home_x"] = df_education_persons["home_geometry"].apply(
        lambda geom: geom.x
    )
    df_education_persons["home_y"] = df_education_persons["home_geometry"].apply(
        lambda geom: geom.y
    )
    ...
    for (lower_bound, upper_bound), types, query_size in zip(
        age_bounds, education_types, query_sizes
    ):
        f_persons = (df_education_persons["age"] >= lower_bound) & (
            df_education_persons["age"] <= upper_bound
        )
        df_candidates = df_edu_candidates[
            df_edu_candidates["education_type"].isin(types)
        ].copy()
        education_coordinates = np.vstack(
            [df_candidates["destination_x"], df_candidates["destination_y"]]
        ).T
        ...
        for i, (ind, dist) in enumerate(zip(indices, distances)):
            ...
            # If enough facilities, apply donut selection
                ...
                donut_ind = ind[
                    (dist >= minimum_selection_bound)
                    & (dist <= maximum_selection_bound)
                ]
                ...
                while len(donut_ind) < query_size and minimum_selection_bound > dist[0]:
                    ...
                    minimum_selection_bound = max(
                        minimum_selection_bound - donut_width * growth_factor, dist[0]
                    )
                    maximum_selection_bound = min(
                        maximum_selection_bound + donut_width * growth_factor, dist[-1]
                    )
                    donut_ind = ind[
                        (dist >= minimum_selection_bound)
                        & (dist <= maximum_selection_bound)
        ...
        print(
            f"INFO: Imputing education locations for age range ({lower_bound}, {upper_bound}]..."
        )
        print(
            f"INFO: % of persons with education facilities not found: {100 * no_fac_count / len(indices):.2f}%"
        )

        # Assign education locations - use actual geometry from candidates to preserve CRS
        df_education_persons.loc[f_persons, "commune_id_edu"] = df_candidates.iloc[
            chosen_indices
        ]["commune_id"].values
        df_education_persons.loc[f_persons, "location_id"] = df_candidates.iloc[
            chosen_indices
        ]["location_id"].values
        df_education_persons.loc[f_persons, "geometry"] = df_candidates.iloc[
            chosen_indices
        ]["geometry"].values

        # Store coordinates for distance calculation
        df_education_persons.loc[f_persons, "education_x"] = df_candidates.iloc[
            chosen_indices
        ]["destination_x"].values
        df_education_persons.loc[f_persons, "education_y"] = df_candidates.iloc[
            chosen_indices
        ]["destination_y"].values
    ...
    print("INFO: Education distance statistics:")
    ...
    df_result = gpd.GeoDataFrame(
        df_education_persons[
            ["person_id", "commune_id_edu", "location_id", "geometry"]
        ].rename(columns={"commune_id_edu": "commune_id"}),
        geometry="geometry",
